...
export const get_character_list = (): CharacterDB[] => {
    ...
    const character_list: CharacterDB[] = Object.values(data).reduce((acc: CharacterDB[], key) => {
        const { id, name, rank } = key;
        if (Number(id) < 7000) {
            acc.push(new CharacterDB(Number(id), name, rank));
        }
        return acc;
    }, []);
...
export const get_character_by_id = (id: number): CharacterDB => {
    let character = get_character_list().filter(c => c.id === id);
...
export const get_character_splash = (id: number | undefined): string => {
